      ...
      mkForSystem = system:
        let
          ...
          yggoverlay-cni-plugin = pkgs.buildGoModule {
            ...
            version =
              if self ? rev
              then lib.substring 0 8 self.rev
              else "dirty";
          ...
          cni-plugins-with-yggoverlay = pkgs.symlinkJoin {
            ...
            paths = [ pkgs.cni-plugins yggoverlay-cni-plugin ];
          };
        in
        {
          packages = {
    ...
    in
      ...
      //
      {
        nixosModules.default = { config, pkgs, lib, ... }:
          ...
          in
          {
            options.virtualisation.containerd.yggoverlayCNI.enable =
              lib.mkEnableOption "Install yggoverlay CNI plugin and point containerd's CNI bin_dir at a merged plugin bundle";

            config = lib.mkIf cfg.enable {
              virtualisation.containerd.enable = true;

              # containerd CRI plugin CNI paths: bin_dir, conf_dir
              # (bin_dir default is /opt/cni/bin; conf_dir default is /etc/cni/net.d). :contentReference[oaicite:1]{index=1}
